...
public class RealmsServer extends ValueObject {
   ...
   private static void sortInvited(RealmsServer server) {
      server.players.sort((playerInfo, playerInfo2) -> {
         return ComparisonChain.start().compareFalseFirst(playerInfo2.isAccepted(), playerInfo.isAccepted()).compare((Comparable)playerInfo.getName().toLowerCase(Locale.ROOT), (Comparable)playerInfo2.getName().toLowerCase(Locale.ROOT)).result();
      });
   }
