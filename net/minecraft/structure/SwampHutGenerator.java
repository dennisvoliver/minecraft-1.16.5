...
public class SwampHutGenerator extends StructurePieceWithDimensions {
   ...
   public boolean generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox boundingBox, ChunkPos chunkPos, BlockPos pos) {
      if (!this.method_14839(world, boundingBox, 0)) {
         ...
         for(k = 2; k <= 7; k += 5) {
            for(l = 1; l <= 5; l += 4) {
               this.fillDownwards(world, Blocks.OAK_LOG.getDefaultState(), l, -1, k, boundingBox);
            }
         }
         ...
         if (!this.hasWitch) {
            ...
            if (boundingBox.contains(new BlockPos(k, l, m))) {
               ...
               witchEntity.initialize(world, world.getLocalDifficulty(new BlockPos(k, l, m)), SpawnReason.STRUCTURE, (EntityData)null, (NbtCompound)null);
   ...
   private void method_16181(ServerWorldAccess serverWorldAccess, BlockBox blockBox) {
      if (!this.hasCat) {
         ...
         if (blockBox.contains(new BlockPos(i, j, k))) {
            ...
            catEntity.initialize(serverWorldAccess, serverWorldAccess.getLocalDifficulty(new BlockPos(i, j, k)), SpawnReason.STRUCTURE, (EntityData)null, (NbtCompound)null);
