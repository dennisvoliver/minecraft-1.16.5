...
public class Structure {
   ...
   public boolean place(ServerWorldAccess world, BlockPos pos, BlockPos blockPos, StructurePlacementData placementData, Random random, int i) {
      if (this.blockInfoLists.isEmpty()) {
         ...
         if ((!list.isEmpty() || !placementData.shouldIgnoreEntities() && !this.entities.isEmpty()) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1) {
            ...
            List<Structure.StructureBlockInfo> list4 = process(world, pos, blockPos, placementData, list);
            ...
            while(true) {
                  ...
                  if (!var18.hasNext()) {
                     ...
                     while(bl && !list2.isEmpty()) {
                        ...
                        while(iterator.hasNext()) {
                           ...
                           for(int p = 0; p < directions.length && !fluidState2.isStill(); ++p) {
                              ...
                              if (fluidState3.getHeight(world, blockPos5) > fluidState2.getHeight(world, blockPos7) || fluidState3.isStill() && !fluidState2.isStill()) {
                           ...
                           if (fluidState2.isStill()) {
                              ...
                              if (block instanceof FluidFillable) {
                                 ((FluidFillable)block).tryFillWithFluid(world, blockPos3, blockState4, fluidState2);
                                 bl = true;
                                 iterator.remove();
                              }
                           }
                     ...
                     if (j <= m) {
                        if (!placementData.shouldUpdateNeighbors()) {
                           ...
                           while(var40.hasNext()) {
                              Pair<BlockPos, NbtCompound> pair = (Pair)var40.next();
                        ...
                        while(iterator.hasNext()) {
                           ...
                           if (!placementData.shouldUpdateNeighbors()) {
                              BlockState blockState3 = world.getBlockState(blockPos7);
                              blockState4 = Block.postProcessState(blockState3, world, blockPos7);
                              if (blockState3 != blockState4) {
                                 world.setBlockState(blockPos7, blockState4, i & -2 | 16);
                              }

                              world.updateNeighbors(blockPos7, blockState4.getBlock());
                           }

                           if (pair2.getSecond() != null) {
                              blockEntity3 = world.getBlockEntity(blockPos7);
               ...
               if (structureBlockInfo.tag != null) {
                  blockEntity3 = world.getBlockEntity(blockPos2);
   ...
   public static List<Structure.StructureBlockInfo> process(WorldAccess world, BlockPos pos, BlockPos blockPos, StructurePlacementData placementData, List<Structure.StructureBlockInfo> list) {
      ...
      while(var6.hasNext()) {
         ...
         for(Iterator iterator = placementData.getProcessors().iterator(); structureBlockInfo2 != null && iterator.hasNext(); structureBlockInfo2 = ((StructureProcessor)iterator.next()).process(world, pos, blockPos, structureBlockInfo, structureBlockInfo2, placementData)) {
         }
